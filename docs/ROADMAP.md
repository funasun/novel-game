...
# 第2次改修『面白さの注入』(2026-07-13 夜)

ユーザー評: 「十五少年漂流記以外いまいち面白くない」。

## 診断 — なぜ十五少年だけ面白いのか

十五少年には**プレイヤーが自分で決める遊びのループ**がある:
「採る → 作る → 建てる → 世界が変わる/拓ける」。複数の目標が同時に開いていて、
次に何をするか自分で選び、行動の結果が世界に見える。

他3作は「光の柱まで歩いて E を押して読む」の一本道。決定的な欠陥は3つ:
1. **パラメータが死んでいる** — 憂鬱/決意/温もり/静けさは動くだけで、何も変えない。
   ゲージが遊びに還元されない（心の管理アクティビティに目的がない）。
2. **世界が反応しない** — 何をしても風景が変わらない。積み上げの喜びがない。
3. **経済も葛藤もない** — 資源の使いみちに選択がない。全部できてしまう＝何も選んでいない。

## 処方 — engine 汎用機構の小追加（4点、全て作品非依存）

- `Effects.flagsOff` — フラグを下ろす（トグル/状態機械/くり返しの営みを可能に）
- `ParamCond {param, gte?, lte?}` + EventTrigger `{type:'param'}` — 心の状態が物語を動かす（閾値イベント・結末分岐）
- `InteractableDef.requiresParam / requiresItems` — 「持っていれば/心が満ちていれば現れる行い」（施し・供え・支払いのゲート）
- `DialogueChoice.requiresFlag / requiresParam` — 心の状態で開く選択肢（足りない選択肢は薄く見せて憧れを作る）

## 各作品の処方

### ハムレット『心の天秤』— パラメータを武器にする
- **狂気の仮面の着脱**（flagsOff トグル）: 自室と回廊で仮面をかぶる/外す。
  仮面ON: 宮廷の**立ち聞きスポット**が開く（狂人の前では皆が口を滑らせる）。suspicion が下がる。
  仮面OFF: ホレイショーとの本音、オフィーリアとの素顔の対話が開く。城内では suspicion が上がる行いも。
- **証拠あつめ → 劇中劇の強度**: 立ち聞きで「毒の行方」「小姓の目撃」「先王の死の矛盾」等の証拠(items)を集める。
  台本書き入れ時に証拠数で選択肢が変わり、劇中劇の王の動揺と doubt 低下量が変わる。
- **決意が果たし合いを開く**（param trigger）: resolve ≥ 65 で「心が定まった」イベント→果たし合い解禁。
  剣の稽古・祈り・語らいの管理ループに**目的**が生まれる。憂鬱 ≥ 85 で「重すぎる夜」イベント（救済の導線）。
- 結末の語りは 決意/疑念 の残量で1行変わる（param trigger で差し込み）。

### クリスマス・キャロル『善意の経済』— 金貨に葛藤を、街に灯りを
- **金貨は足りない**: 朝の善行に加えて、街に施しの機会を大幅増設（requiresParam coins ゲート）。
  全部はできない残高 → どこに使うかの選択が生まれる。
- **取り立ての誘惑**: debtors から取り立てれば金貨は増えるが warmth が下がる（スクルージ的ジレンマ）。
- **街のぬくもりが見える**（param trigger → flags → Scene 描画）: warmth の閾値で
  窓の灯りが増える/聖歌隊が現れる/広場のツリーに灯が入る。行いが風景になる。
- **朝の結末分岐**: warmth 到達度で最終ナレーションが3段変化。
### 高瀬舟『提灯と頼まれごと』— 静けさを灯す
- **蛍を掬って提灯に灯す**: 岸の蛍(cooldown資源) → 舫いの消えた灯籠に灯す(requiresItems) → 灯った灯籠が Scene に残る。静けさ↑。
- **岸の小さな頼まれごと**: 舫いごとに1つ（地蔵に水/高札の迷い文/杭の緩んだ綱）。喜助の「知足」と響き合う小さな善。
- 結末の独白は 問い/静けさ の満ち方で変わる（param trigger）。

## 検証
各作品 E2E（新ループ＋既存回帰）→ commit → 最後に gh-pages。ハーネスは worker-RAF shim 方式。
